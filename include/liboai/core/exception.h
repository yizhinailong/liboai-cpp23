...
namespace liboai::exception {
    ...
    public:
        ...
        OpenAIException(OpenAIException&& rhs) noexcept
            : error_type_(rhs.error_type_), data_(std::move(rhs.data_)), locale_(std::move(rhs.locale_)) {
            this->fmt_str_ = (this->locale_ + ": " + this->data_ + " (" + this->GetETypeString(this->error_type_) + ")");
        }
        ...
        static constexpr const char* GetETypeString(EType type) noexcept {
    ...
    public:
        ...
        OpenAIRateLimited(std::string_view data, EType error_type, std::string_view locale) noexcept
            : error_type_(error_type), data_(data), locale_(locale) {
            this->fmt_str_ = (this->locale_ + ": " + this->data_ + " (" + this->GetETypeString(this->error_type_) + ")");
        }
